...
public class BuscadorCaminos {
    ...
    private int llamadasSinPoda = 0;
    private int llamadasConPoda = 0;
    private long tiempoSinPoda = 0;
    ...
    public void buscarSinPoda() {
        ...
        buscar(0, 0, 0, new ArrayList<>(), false);
        tiempoSinPoda = System.currentTimeMillis() - inicio;
    }
    ...
    public void buscarConPoda() {
        ...
        llamadasConPoda = 0;
        long inicio = System.currentTimeMillis();
        buscar(0, 0, 0, new ArrayList<>(), true);
        tiempoConPoda = System.currentTimeMillis() - inicio;
    }

    private void buscar(int i, int j, int suma, List<int[]> camino, boolean usarPoda) {
        if (i >= grilla.getFilas() || j >= grilla.getColumnas()) return;

        if (usarPoda) llamadasConPoda++;
        else llamadasSinPoda++;

        suma += grilla.getValor(i, j);
        camino.add(new int[]{i, j});

        if (i == grilla.getFilas() - 1 && j == grilla.getColumnas() - 1) {
        	if (true) caminosValidos.add(new ArrayList<>(camino));
            camino.remove(camino.size() - 1);
            return;
        }

        int restantes = (grilla.getFilas() - 1 - i) + (grilla.getColumnas() - 1 - j);
        if (usarPoda && Math.abs(suma) > restantes) {
            camino.remove(camino.size() - 1);
            return;
        }

        buscar(i + 1, j, suma, camino, usarPoda);
        buscar(i, j + 1, suma, camino, usarPoda);
        camino.remove(camino.size() - 1);
    }

    public List<List<int[]>> getCaminosValidos() { return caminosValidos; }
    public int getLlamadasSinPoda() { return llamadasSinPoda; }
